...
assert.equal(D.encounters.length,105);assert.equal(new Set(D.encounters.map(x=>x.id)).size,105);
...
for(let area=0;area<5;area++)for(let seed=1;seed<=60;seed++){
 const g=make(seed*7919);g.state.unlocked=5;g.start(area);const r=g.state.run,ids=r.rooms;
...
assert.equal(routes.size,300);assert.ok(scribes.size>5);
...
for(const e of D.encounters)for(const left of [true,false]){
 const g=make(512);g.state.unlocked=5;g.start(e.area??0);g.state.run.rooms=[e.id,'boss'];g.state.gold=left?100:0;g.state.hp=100;
...
console.log('105 authored encounters, 300 route structures, contextual pools, ordering, stable saves, both choices, shop restrictions and delayed help verified.');
