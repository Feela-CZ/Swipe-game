...
class Game {
 ...
 fresh(){
  ...
  return {version:4,heroId:'male',heroChosen:false,heroName:'',level:1,xp:0,points:30,levelNotice:{from:1,to:1,hp:0,points:30,initial:true},growth:{might:0,grit:0,agility:0,intelligence:0,luck:0,perception:0},
   ...
   unlocked:1,run:null,journal:[],flags:{},settings:{sound:false,volume:.55,music:true,musicVolume:.3,speed:1,mapLabels:true},lastReport:null,
 ...
 migrate(raw){
  ...
  if(raw.version===3||raw.version===4||raw.version===5){
   ...
   s.settings={sound:raw.settings?.sound===true,volume:Number.isFinite(raw.settings?.volume)?clamp(raw.settings.volume,0,1):.55,music:raw.settings?.music!==false,musicVolume:Number.isFinite(raw.settings?.musicVolume)?clamp(raw.settings.musicVolume,0,1):.3,speed:raw.settings?.speed===2?2:1,mapLabels:raw.settings?.mapLabels!==false};
